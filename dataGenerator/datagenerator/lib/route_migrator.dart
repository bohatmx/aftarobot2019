...
import 'package:datagenerator/route_migration.dart';
...
class _RouteMigratorState extends State<RouteMigrator>
    implements RouteMigrationListener, AssociationMigrationListener {
  ...
  void _getOldRoutes() async {
    var tempRoutes = await RouteMigration.getOldRoutes();
  ...
  void _migrateAss() async {
    ...
    _startTimer();
    await RouteMigration.migrateAssociations(this);
    RouteDTO mRoute;
    routes.forEach((r) {
    ...
    try {
      ...
      await RouteMigration.primeQueriesToGetIndexingLink(
          route: mRoute,
          landmark: mRoute.spatialInfos.elementAt(0).fromLandmark);
      print('_RouteMigratorState._migrateRoutes ... start the real work!!');
      await RouteMigration.migrateRoutes(routes: clonedRoutes, listener: this);
      print(
